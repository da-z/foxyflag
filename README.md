...
[Download the signed .xpi](https://github.com/da-z/foxyflag/releases/download/v1.0.1/foxyflag-signed.xpi) and open it in Firefox.

## How it works

1. When you navigate to a site, FoxyFlag resolves the domain to an IP address using Firefox's `dns.resolve()` API
2. If the resolved IP is public, the IP is looked up via [ip-api.com](http://ip-api.com) to determine the hosting country
...
If the country can't be determined, or the site resolves to a local/private address, no icon is shown.
